...
<template>
  ...
  >
    <div class="flex justify-between content-between py-6 px-4 md:px-6 xl:px-7.5">
      <h4 class="text-xl font-bold text-black dark:text-white">{{ props.label }}</h4>
      <slot></slot>
    </div>

    <!-- Table Header -->
    <div 
      class="flex justify-between border-t border-stroke py-4.5  dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5"
    >
      <div  v-for="headers in props.tableHeaders" 
      :key="headers.text"
      class="col-span-1 hidden items-center sm:flex">
        <p class="font-medium">{{ headers.text }}</p>
      </div>
    ...
    >
      <div class="col-span-2 flex items-center">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
          <div class="h-12.5 w-15 rounded-md">
            <img :src="product.src" :alt="`Product: ${product.alt}`" />
          </div>
          <p class="col-span-2 text-sm font-medium text-black dark:text-white">{{ product.name }}</p>
        </div>
      </div>
      ...
      <div v-if="isAction" class="flex items-center space-x-3.5">
