<template>
  <div v-if="open" class="fixed z-50 inset-0 overflow-y-auto  ">
    <div class="flex items-end justify-center min-h-screen text-center sm:block">
      ...
      <div
        class="inline-block align-bottom bg-[rgb(220,214,205)] text-left overflow-hidden shadow-xl
         transform transition-all sm:align-middle  w-[96vw] h-[70vh] overflow-y-auto  "
        role="dialog"
        aria-modal="true"
      ...
      >
        <div class="hidden sm:block absolute top-4 right-4  ">
          <button
            ...
            class="rounded-md text-black-400 hover:text-gray-500 focus:outline-none
        ...
        <div class="flex overflow-auto m-auto ">
          <div class="w-3/5">
            <!-- <video autoplay="true" muted="true" loop="true" controls="">
              <source
                :src="currentVideo"
                type="video/mp4"
              />
          ...
          <div class="w-2/5 bg-[rgb(220,214,205)] px-10 ">
            <div class="text-lg pt-10 ">
              <h2 class="uppercase text-[28px] text-[#606060] font-semibold pb-3">Product Video</h2>
            </div>
            <div class="flex pt-4" v-for="(vdo, index) in videos" :key="index" :class="{'border-b-2 pb-5 border-[#978b7a7d]': index === 0 }" >
                <h4 :class="{ 'text-orange': currentVideo === vdo?.src }" class=" text-[18px]">{{ vdo?.name }}</h4>
                <a
                  class="text-h3Font cursor-pointer border  border-black hover:bg-orange hover:border-0   leading-6 ease-in-out hover:text-white rounded-3xl ml-auto px-4 py-auto font-light text-[13px]"
                  @click="setVideo(vdo?.src)"
                  data-toggle="modal"
                  > Watch</a
                >
              </div>
...
const renderVideo = (videoSrc) => {
  videoContainer.value = `<video  width="100%" height="100%" class="w-full h-full" autoplay="true" muted="true" loop="true" controls=""><source src="${videoSrc}" type="video/mp4" /></video>`;
};
